            ...
            if os.path.exists(startup_path):
                try:
                    for item in os.listdir(startup_path):
                        ...
                        if os.path.isfile(item_path) and item.lower().endswith(('.lnk', '.exe', '.bat', '.cmd', '.vbs', '.ps1')):
            ...
            if os.path.exists(disabled_folder):
                try:
                    for item in os.listdir(disabled_folder):
                        ...
                        if os.path.isfile(item_path) and item.lower().endswith(('.lnk', '.exe', '.bat', '.cmd', '.vbs', '.ps1')):
                            existing = any(p["filename"] == item and p["startup_path"] == startup_path for p in startup_programs)
                            if not existing:
                                startup_programs.append({
                                    "type": "folder",
                                    "display_name": os.path.splitext(item)[0],
...
class ModernSystemMonitor:
    ...
    def action_disable_telemetry(self):
        ...
        if errors:
            self.optimize_status_label.configure(text=f"Отключено {disabled} из {disabled+len(errors)} служб", text_color="orange")
            messagebox.showwarning("Предупреждение", f"Отключено {disabled} служб.\nНе удалось отключить: {', '.join(errors)}")
        else:
